...
export default function AmirDialog({
    open,
    onOpenChange,
    onSuccess,
    initialData,
    ...
    const onSubmit: SubmitHandler<FormValues> = async (data) => {
        setLoading(true);
        try {
            const url = initialData ? `/api/kurum-amirleri/${initialData.id}` : "/api/kurum-amirleri";
    ...
    return (
        <Modal
            ...
            title={initialData ? "Amir Düzenle" : "Yeni Kayıt Ekle"}
            size="lg"
        >
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <Input label="Ad Soyad" {...register("ad_soyad")} error={errors.ad_soyad?.message} />
                    <Input label="Kurum Adı" {...register("kurum_adi")} error={errors.kurum_adi?.message} />
                </div>
