...
import { Edit2, Trash2, Phone, Mail, Building, User, Grid, List as ListIcon } from "lucide-react";
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="relative w-full sm:w-96">
                    <input
                        type="text"
                        placeholder="İsim, Kurum veya Ünvan Ara..."
                        value={search}
            ...
            {loading ? (
                        ...
                        <table className="w-full text-sm text-left">
                            <thead className="text-xs text-slate-500 uppercase bg-slate-50/50 border-b border-slate-200">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 font-semibold">Adı Soyadı / Ünvan</th>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {items.map((amir) => (
                                    <tr key={amir.id} className="hover:bg-slate-50/80 transition-colors group">
                                        ...
                                        <td className="px-6 py-4">
                                            <div>
                                                <div className="font-semibold text-slate-900">{amir.ad_soyad}</div>
                                                <div className="text-[11px] text-blue-600 font-medium uppercase mt-0.5">{amir.unvan || "-"}</div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                <Building className="w-3.5 h-3.5 text-slate-400" />
                                                <span className="text-slate-700 font-medium">{amir.kurum_adi}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col gap-1.5">
                                                {amir.gsm && (
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="font-mono text-xs text-slate-700">{amir.gsm}</span>
                                                    </div>
                                                )}
                                                {amir.email && (
                                                    <div className="flex items-center gap-2">
                                                        <Mail className="w-3.5 h-3.5 text-slate-400" />
                                                        <a href={`mailto:${amir.email}`} className="text-xs text-blue-600 hover:underline">{amir.email}</a>
                                                    </div>
                                                )}
                                            </div>
                                        </td>
                                        <td className="px-4 py-4 text-right">
                                            <div className="flex justify-end gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all duration-200 transform translate-x-2 sm:group-hover:translate-x-0">
                                                <button onClick={() => { setEditItem(amir); setIsDialogOpen(true); }} className="p-2 bg-white border border-slate-200 shadow-sm hover:shadow hover:border-blue-300 text-slate-500 hover:text-blue-600 rounded-lg transition-all" title="Düzenle">
                                                    <Edit2 className="w-4 h-4" />
                                                </button>
                                                <button onClick={() => handleDelete(amir.id)} className="p-2 bg-white border border-slate-200 shadow-sm hover:shadow hover:border-red-300 text-slate-500 hover:text-red-600 rounded-lg transition-all" title="Sil">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                    {items.map((amir) => (
                        <div key={amir.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-lg hover:border-blue-200 transition-all duration-300 relative group overflow-hidden">
                            ...
                            <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0 flex gap-1 z-10">
                                <button
                                    onClick={() => { setEditItem(amir); setIsDialogOpen(true); }}
                            ...
                            <div className="flex flex-col items-center text-center relative z-0 mt-4">
                                ...
                                <h3 className="font-bold text-slate-900 text-lg leading-tight mb-1">{amir.ad_soyad}</h3>
                                <div className="text-xs font-bold text-blue-600 uppercase tracking-wide mb-3 px-2 py-0.5 bg-blue-50 rounded-full">
                                    {amir.unvan || "Unvan Belirtilmedi"}
                                </div>

                                <div className="w-full space-y-2 border-t border-slate-50 pt-3">
                                    <div className="flex items-center justify-center gap-2 text-sm text-slate-600 font-medium">
                                        <Building className="w-3.5 h-3.5 text-slate-400" />
                                        <span className="truncate max-w-[200px]" title={amir.kurum_adi}>{amir.kurum_adi}</span>
                                    </div>
                                    ...
                                    {amir.email && (
                                        <div className="flex items-center justify-center gap-2 text-xs text-slate-500">
                                            ...
                                            <a href={`mailto:${amir.email}`} className="truncate max-w-[200px] hover:text-blue-600 transition-colors">{amir.email}</a>
                                        </div>
                                    )}
                                </div>
                            </div>
