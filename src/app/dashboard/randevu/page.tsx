...
import DailyProtocolPrint from "@/components/makam-randevu/DailyProtocolPrint";
...
export default function RandevuPage() {
  ...
  const handleAdd = (date?: Date, time?: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/randevu/${sonucNotuTarget.id}`, {
        ...
        body: JSON.stringify({
          ...
          durum: APPOINTMENT_STATUS.COMPLETED.id,
  ...
  return (
    <>
      <div className="print:hidden w-full max-w-[1600px] mx-auto space-y-6 px-4">
        ...
        <div className="relative overflow-hidden bg-gradient-to-r from-slate-900 via-blue-900 to-slate-900 animate-gradient-x rounded-2xl p-6 shadow-2xl border border-white/5">
          ...
          <div className="relative z-10 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-4">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {/* Date Navigation - Both List and Calendar views */}
              {dateRange === "today" && (
                            ...
                            >
                              ...
                              <td className="px-4 py-4 text-right">
                                <div className="flex items-center justify-end gap-1" onClick={(e) => e.stopPropagation()}>
                                  ...
                                  {/* Tamamlandı - Talimat Ver butonu */}
                                  {getStatusConfig(randevu.durum).id === APPOINTMENT_STATUS.COMPLETED.id && (
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        handleTalimatOlustur(randevu);
                                      }}
        ...
        {/* Modal */}
        {
          modalOpen && (
            <RandevuModal
              ...
              existingAppointments={randevular}
            />
          )
        }
        {/* Postpone Modal */}
      ...
      <DailyProtocolPrint
        date={new Date(filterTarih)}
        appointments={viewMode === "takvim" ? getFilteredStats() : randevular.filter(r => r.tarih === filterTarih)}
      />
    </>
  );
}
