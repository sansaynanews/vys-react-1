...
const talepUpdateSchema = z.object({
...
export async function PUT(
    ...
    try {
        ...
        const validated = talepUpdateSchema.parse(body);

        const data: any = { ...validated };
        if (data.tarih) data.tarih = new Date(data.tarih);

        const item = await prisma.gorusme_talepleri.update({
...
export async function DELETE(
    ...
    try {
        ...
        const { id: paramId } = await params;
        await prisma.gorusme_talepleri.delete({ where: { id: parseInt(paramId) } });
