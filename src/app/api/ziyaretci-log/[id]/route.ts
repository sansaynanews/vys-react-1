...
export async function GET(
  ...
  try {
    ...
    const log = await prisma.ziyaretci_log.findUnique({
      where: { id },
    });
    ...
    return NextResponse.json(
      { error: "Log getirilemedi" },
      { status: 500 }
    );
  }
}

// PUT - Ziyaretçi log güncelle
export async function PUT(
  ...
  try {
    ...
    await prisma.ziyaretci_log.update({
      where: { id },
      data: {
    ...
    return NextResponse.json({
      message: "Ziyaretçi logu başarıyla güncellendi",
    });
  } catch (error) {
    ...
    console.error("Ziyaretci Log PUT Error:", error);
    return NextResponse.json(
      { error: "Log güncellenemedi" },
      { status: 500 }
    );
  }
}

// DELETE - Ziyaretçi log sil
export async function DELETE(
  ...
  try {
    ...
    await prisma.ziyaretci_log.delete({
      where: { id },
    });

    return NextResponse.json({
      message: "Ziyaretçi logu başarıyla silindi",
    });
  } catch (error) {
    console.error("Ziyaretci Log DELETE Error:", error);
    return NextResponse.json(
      { error: "Log silinemedi" },
      { status: 500 }
    );
  }
}
